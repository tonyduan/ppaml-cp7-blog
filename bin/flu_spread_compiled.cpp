...
const int _BURN_IN = 199900;
int _tot_round = -199900;
...
Gamma Gamma140187620418000;
Gamma Gamma140187620418176;
Gaussian Gaussian140187622445056;
BooleanDistrib BooleanDistrib140187622448256;
BooleanDistrib BooleanDistrib140187619405488;
Gaussian Gaussian140187619412224;
Gaussian Gaussian140187619451920;
...
void _init_storage()
{
  ...
  for (int t = 0;t<103;t++)
  {
    for (int p = 0;p<432;p++)
    {
      _mem_spatial_edge[t][p]=new _Var_spatial_edge(t, p);
    }
  ...
  Gamma140187620418000.init(1.05000000,0.50000000);
  Gamma140187620418176.init(3.00000000,0.10000000);
...
void _init_world()
{
  ...
  for (int t = 0;t<103;t++)
  for (int p = 0;p<432;p++)
  _util_set_evidence<char>(_mem_spatial_edge[t][p],1);
...
{
  return Gamma140187620418000.loglikeli(val);
}
double _Var_rho::getcachelikeli()
{
  auto _t_val = getcache();
  return Gamma140187620418000.loglikeli(_t_val);
}
void _Var_rho::sample()
{
  val=Gamma140187620418000.gen();
}
void _Var_rho::sample_cache()
{
  cache_val=Gamma140187620418000.gen();
}
...
{
  return Gamma140187620418176.loglikeli(val);
}
double _Var_tau1::getcachelikeli()
{
  auto _t_val = getcache();
  return Gamma140187620418176.loglikeli(_t_val);
}
void _Var_tau1::sample()
{
  val=Gamma140187620418176.gen();
}
void _Var_tau1::sample_cache()
{
  cache_val=Gamma140187620418176.gen();
}
...
{
  return Gaussian140187622445056.init(__fixed_priors(c,t),__fixed_D[c]),Gaussian140187622445056.loglikeli(val);
}
double _Var_y::getcachelikeli()
{
  auto _t_val = getcache();
  return Gaussian140187622445056.init(__fixed_priors(c,t),__fixed_D[c]),Gaussian140187622445056.loglikeli(_t_val);
}
void _Var_y::sample()
{
  val=(Gaussian140187622445056.init(__fixed_priors(c,t),__fixed_D[c]),Gaussian140187622445056.gen());
}
void _Var_y::sample_cache()
{
  cache_val=(Gaussian140187622445056.init(__fixed_priors(c,t),__fixed_D[c]),Gaussian140187622445056.gen());
}
...
{
  return BooleanDistrib140187622448256.init(exp(_mem_tau1->getval()*_mem_y[c][t]->getval()*_mem_y[c][__fixed_toWeek(t-1)]->getval())),BooleanDistrib140187622448256.loglikeli(val);
}
double _Var_temporal_edge::getcachelikeli()
{
  auto _t_val = getcache();
  return BooleanDistrib140187622448256.init(exp(_mem_tau1->getcache()*_mem_y[c][t]->getcache()*_mem_y[c][__fixed_toWeek(t-1)]->getcache())),BooleanDistrib140187622448256.loglikeli(_t_val);
}
void _Var_temporal_edge::sample()
{
  val=(BooleanDistrib140187622448256.init(exp(_mem_tau1->getval()*_mem_y[c][t]->getval()*_mem_y[c][__fixed_toWeek(t-1)]->getval())),BooleanDistrib140187622448256.gen());
}
void _Var_temporal_edge::sample_cache()
{
  cache_val=(BooleanDistrib140187622448256.init(exp(_mem_tau1->getcache()*_mem_y[c][t]->getcache()*_mem_y[c][__fixed_toWeek(t-1)]->getcache())),BooleanDistrib140187622448256.gen());
}
...
{
  return BooleanDistrib140187619405488.init(exp(_mem_tau1->getval()*_mem_rho->getval()*_mem_y[__fixed_toCounty(toInt(__fixed_spatial_obs(p,0)))][t]->getval()*_mem_y[__fixed_toCounty(toInt(__fixed_spatial_obs(p,1)))][t]->getval())),BooleanDistrib140187619405488.loglikeli(val);
}
double _Var_spatial_edge::getcachelikeli()
{
  auto _t_val = getcache();
  return BooleanDistrib140187619405488.init(exp(_mem_tau1->getcache()*_mem_rho->getcache()*_mem_y[__fixed_toCounty(toInt(__fixed_spatial_obs(p,0)))][t]->getcache()*_mem_y[__fixed_toCounty(toInt(__fixed_spatial_obs(p,1)))][t]->getcache())),BooleanDistrib140187619405488.loglikeli(_t_val);
}
void _Var_spatial_edge::sample()
{
  val=(BooleanDistrib140187619405488.init(exp(_mem_tau1->getval()*_mem_rho->getval()*_mem_y[__fixed_toCounty(toInt(__fixed_spatial_obs(p,0)))][t]->getval()*_mem_y[__fixed_toCounty(toInt(__fixed_spatial_obs(p,1)))][t]->getval())),BooleanDistrib140187619405488.gen());
}
void _Var_spatial_edge::sample_cache()
{
  cache_val=(BooleanDistrib140187619405488.init(exp(_mem_tau1->getcache()*_mem_rho->getcache()*_mem_y[__fixed_toCounty(toInt(__fixed_spatial_obs(p,0)))][t]->getcache()*_mem_y[__fixed_toCounty(toInt(__fixed_spatial_obs(p,1)))][t]->getcache())),BooleanDistrib140187619405488.gen());
}
void _Var_spatial_edge::active_edge()
{
  _mem_rho->add_contig(this);
  _mem_tau1->add_contig(this);
  _mem_y[__fixed_toCounty(toInt(__fixed_spatial_obs(p,0)))][t]->add_contig(this);
  _mem_y[__fixed_toCounty(toInt(__fixed_spatial_obs(p,1)))][t]->add_contig(this);
  _mem_rho->add_child(this);
  _mem_tau1->add_child(this);
  _mem_y[__fixed_toCounty(toInt(__fixed_spatial_obs(p,0)))][t]->add_child(this);
  _mem_y[__fixed_toCounty(toInt(__fixed_spatial_obs(p,1)))][t]->add_child(this);
}
void _Var_spatial_edge::remove_edge()
{
  _mem_rho->erase_contig(this);
  _mem_tau1->erase_contig(this);
  _mem_y[__fixed_toCounty(toInt(__fixed_spatial_obs(p,0)))][t]->erase_contig(this);
  _mem_y[__fixed_toCounty(toInt(__fixed_spatial_obs(p,1)))][t]->erase_contig(this);
  _mem_rho->erase_child(this);
  _mem_tau1->erase_child(this);
  _mem_y[__fixed_toCounty(toInt(__fixed_spatial_obs(p,0)))][t]->erase_child(this);
  _mem_y[__fixed_toCounty(toInt(__fixed_spatial_obs(p,1)))][t]->erase_child(this);
}
...
{
  return Gaussian140187619412224.init(__fixed_beta1*__fixed_covariates1(c,t)+__fixed_beta2*__fixed_covariates2(c,t)+_mem_y[c][t]->getval(),0.000000),Gaussian140187619412224.loglikeli(val);
}
double _Var_logit::getcachelikeli()
{
  auto _t_val = getcache();
  return Gaussian140187619412224.init(__fixed_beta1*__fixed_covariates1(c,t)+__fixed_beta2*__fixed_covariates2(c,t)+_mem_y[c][t]->getcache(),0.000000),Gaussian140187619412224.loglikeli(_t_val);
}
void _Var_logit::sample()
{
  val=(Gaussian140187619412224.init(__fixed_beta1*__fixed_covariates1(c,t)+__fixed_beta2*__fixed_covariates2(c,t)+_mem_y[c][t]->getval(),0.000000),Gaussian140187619412224.gen());
}
void _Var_logit::sample_cache()
{
  cache_val=(Gaussian140187619412224.init(__fixed_beta1*__fixed_covariates1(c,t)+__fixed_beta2*__fixed_covariates2(c,t)+_mem_y[c][t]->getcache(),0.000000),Gaussian140187619412224.gen());
}
