...
Gamma Gamma140584268375632;
Gamma Gamma140584268382416;
Gamma Gamma140584268382752;
Gaussian Gaussian140584268383088;
Gaussian Gaussian140584268383424;
Gaussian Gaussian140584268387056;
BooleanDistrib BooleanDistrib140584266435424;
BooleanDistrib BooleanDistrib140584269385424;
Gaussian Gaussian140584269389104;
Gaussian Gaussian140584269395712;
...
void _init_storage()
{
  ...
  _mem_temporal_edge.resize(2,277);
  ...
  Gamma140584268375632.init(1.05000000,0.50000000);
  Gamma140584268382416.init(3.00000000,0.10000000);
  Gamma140584268382752.init(10.00000000,0.10000000);
  Gaussian140584268383088.init(0.000000,10.00000000);
  Gaussian140584268383424.init(0.000000,10.00000000);
...
{
  return Gamma140584268375632.loglikeli(val);
}
double _Var_rho::getcachelikeli()
{
  auto _t_val = getcache();
  return Gamma140584268375632.loglikeli(_t_val);
}
void _Var_rho::sample()
{
  val=Gamma140584268375632.gen();
}
void _Var_rho::sample_cache()
{
  cache_val=Gamma140584268375632.gen();
}
...
{
  return Gamma140584268382416.loglikeli(val);
}
double _Var_tau1::getcachelikeli()
{
  auto _t_val = getcache();
  return Gamma140584268382416.loglikeli(_t_val);
}
void _Var_tau1::sample()
{
  val=Gamma140584268382416.gen();
}
void _Var_tau1::sample_cache()
{
  cache_val=Gamma140584268382416.gen();
}
...
{
  return Gamma140584268382752.loglikeli(val);
}
double _Var_tau2::getcachelikeli()
{
  auto _t_val = getcache();
  return Gamma140584268382752.loglikeli(_t_val);
}
void _Var_tau2::sample()
{
  val=Gamma140584268382752.gen();
}
void _Var_tau2::sample_cache()
{
  cache_val=Gamma140584268382752.gen();
}
...
{
  return Gaussian140584268383088.loglikeli(val);
}
double _Var_beta1::getcachelikeli()
{
  auto _t_val = getcache();
  return Gaussian140584268383088.loglikeli(_t_val);
}
void _Var_beta1::sample()
{
  val=Gaussian140584268383088.gen();
}
void _Var_beta1::sample_cache()
{
  cache_val=Gaussian140584268383088.gen();
}
...
{
  return Gaussian140584268383424.loglikeli(val);
}
double _Var_beta2::getcachelikeli()
{
  auto _t_val = getcache();
  return Gaussian140584268383424.loglikeli(_t_val);
}
void _Var_beta2::sample()
{
  val=Gaussian140584268383424.gen();
}
void _Var_beta2::sample_cache()
{
  cache_val=Gaussian140584268383424.gen();
}
...
{
  return Gaussian140584268387056.init(0.000000,__fixed_D[c]),Gaussian140584268387056.loglikeli(val);
}
double _Var_y::getcachelikeli()
{
  auto _t_val = getcache();
  return Gaussian140584268387056.init(0.000000,__fixed_D[c]),Gaussian140584268387056.loglikeli(_t_val);
}
void _Var_y::sample()
{
  val=(Gaussian140584268387056.init(0.000000,__fixed_D[c]),Gaussian140584268387056.gen());
}
void _Var_y::sample_cache()
{
  cache_val=(Gaussian140584268387056.init(0.000000,__fixed_D[c]),Gaussian140584268387056.gen());
}
...
{
  return BooleanDistrib140584266435424.init(exp(_mem_tau1->getval()*_mem_y[c][t1]->getval()*_mem_y[c][t2]->getval())),BooleanDistrib140584266435424.loglikeli(val);
}
double _Var_temporal_edge::getcachelikeli()
{
  auto _t_val = getcache();
  return BooleanDistrib140584266435424.init(exp(_mem_tau1->getcache()*_mem_y[c][t1]->getcache()*_mem_y[c][t2]->getcache())),BooleanDistrib140584266435424.loglikeli(_t_val);
}
void _Var_temporal_edge::sample()
{
  val=(BooleanDistrib140584266435424.init(exp(_mem_tau1->getval()*_mem_y[c][t1]->getval()*_mem_y[c][t2]->getval())),BooleanDistrib140584266435424.gen());
}
void _Var_temporal_edge::sample_cache()
{
  cache_val=(BooleanDistrib140584266435424.init(exp(_mem_tau1->getcache()*_mem_y[c][t1]->getcache()*_mem_y[c][t2]->getcache())),BooleanDistrib140584266435424.gen());
}
...
{
  return BooleanDistrib140584269385424.init(exp(_mem_tau1->getval()*_mem_rho->getval()*_mem_y[c1][t]->getval()*_mem_y[c2][t]->getval())),BooleanDistrib140584269385424.loglikeli(val);
}
double _Var_spatial_edge::getcachelikeli()
{
  auto _t_val = getcache();
  return BooleanDistrib140584269385424.init(exp(_mem_tau1->getcache()*_mem_rho->getcache()*_mem_y[c1][t]->getcache()*_mem_y[c2][t]->getcache())),BooleanDistrib140584269385424.loglikeli(_t_val);
}
void _Var_spatial_edge::sample()
{
  val=(BooleanDistrib140584269385424.init(exp(_mem_tau1->getval()*_mem_rho->getval()*_mem_y[c1][t]->getval()*_mem_y[c2][t]->getval())),BooleanDistrib140584269385424.gen());
}
void _Var_spatial_edge::sample_cache()
{
  cache_val=(BooleanDistrib140584269385424.init(exp(_mem_tau1->getcache()*_mem_rho->getcache()*_mem_y[c1][t]->getcache()*_mem_y[c2][t]->getcache())),BooleanDistrib140584269385424.gen());
}
...
{
  return Gaussian140584269389104.init(0.000000,1.00000000/_mem_tau2->getval()),Gaussian140584269389104.loglikeli(val);
}
double _Var_noise::getcachelikeli()
{
  auto _t_val = getcache();
  return Gaussian140584269389104.init(0.000000,1.00000000/_mem_tau2->getcache()),Gaussian140584269389104.loglikeli(_t_val);
}
void _Var_noise::sample()
{
  val=(Gaussian140584269389104.init(0.000000,1.00000000/_mem_tau2->getval()),Gaussian140584269389104.gen());
}
void _Var_noise::sample_cache()
{
  cache_val=(Gaussian140584269389104.init(0.000000,1.00000000/_mem_tau2->getcache()),Gaussian140584269389104.gen());
}
